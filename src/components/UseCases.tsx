...
const UseCases = () => {
  ...
  return (
    <div className="py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      >
        <motion.div 
          className="absolute w-[800px] h-[800px] -top-[400px] -right-[400px] rounded-full bg-gradient-to-br from-violet-100/20 to-cyan-100/20 blur-3xl"
          style={{ y: bgY1 }}
        />
        <motion.div 
          className="absolute w-[600px] h-[600px] top-[20%] -left-[300px] rounded-full bg-gradient-to-br from-fuchsia-100/20 to-rose-100/20 blur-3xl"
          style={{ y: bgY2 }}
        />
      </motion.div>
      ...
      <div className="max-w-7xl mx-auto relative">
            ...
            <motion.div className="flex gap-2">
              ...
              key={tab}
                ...
                >
                  {activeTab === tab && (
                    <motion.div
                      className="absolute inset-0 bg-gradient-to-r from-violet-50 to-cyan-50 rounded-xl -z-10"
                      layoutId="activeTab"
                      transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                    />
                  )}
                  {tab === 'inbound' ? 'Inbound Solutions' : 'Outbound Solutions'}
                </motion.button>
              ))}
            </motion.div>
          </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {(activeTab === 'inbound' ? inboundCases : outboundCases).map((useCase, index) => (
              <CardWrapper key={index} index={index}>
                ...
                >
                  ...
                  <div className="relative bg-white/90 backdrop-blur-sm rounded-2xl p-8 shadow-lg shadow-gray-100/50 border border-gray-100 h-full transition-all duration-300 group-hover:-translate-y-1 group-hover:bg-white">
                    {/* Enhanced Icon */}
                    <div className={`w-16 h-16 rounded-xl bg-gradient-to-r ${useCase.gradient} p-0.5 mb-8 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}>
                      <div className="w-full h-full bg-white rounded-[10px] flex items-center justify-center">
                        <img src={useCase.icon} alt={useCase.title} className="w-8 h-8" />
                      </div>
                    </div>
                ...
                {useCase.description}
                    ...
                    <div className="space-y-4">
                      {useCase.features.map((feature, i) => (
                        <motion.div
                          key={i}
                          className="flex items-center justify-between gap-4 p-3 rounded-lg bg-gray-50/80 backdrop-blur-sm group-hover:bg-gradient-to-r group-hover:from-gray-50 group-hover:to-white transition-all duration-300"
                        ...
                        >
                          ...
                          <span className={`text-sm font-medium bg-gradient-to-r ${useCase.gradient} text-transparent bg-clip-text`}>
                            {feature.value}
                          </span>
                        </motion.div>
          ))}
