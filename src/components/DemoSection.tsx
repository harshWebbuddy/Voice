  ...
  return (
    <div className="py-24 relative overflow-hidden bg-gradient-to-b from-white to-gray-50">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-gradient-to-r from-emerald-100/30 to-teal-100/30 rounded-full blur-3xl opacity-50 translate-x-1/2 translate-y-1/2" />
      </div>
          ...
          {demos.map((demo, index) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-r rounded-3xl transition-all duration-300 group-hover:opacity-100 opacity-0 p-[1px]">
                <div className={`absolute inset-0 bg-gradient-to-r ${demo.gradient} rounded-3xl opacity-10`} />
              </div>
              ...
              <div className="relative bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100 transition-all duration-300 group-hover:-translate-y-1">
                ...
                <div className="flex items-start gap-4 mb-6">
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-r ${demo.gradient} p-0.5 shrink-0`}>
                  ...
                  <div>
                    ...
                    <p className={`text-sm bg-gradient-to-r ${demo.gradient} text-transparent bg-clip-text font-medium`}>
                      {demo.category}
                    </p>
                  </div>
                </div>
                ...
                <div className="h-16 mb-6 flex items-center justify-between gap-0.5">
                  {Array.from({ length: 40 }).map((_, i) => (
                    <motion.div
                      ...
                      animate={activeDemo === demo.id && isPlaying ? {
                        height: ['20%', '100%', '20%'],
                        transition: {
                          duration: 1,
                          repeat: Infinity,
                    ...
                    {activeDemo === demo.id && isPlaying ? (
                      <>
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24">
                          <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 5v14M16 5v14"/>
                        </svg>
                        Pause Demo
                      </>
                    ) : (
                      <>
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24">
                          <path fill="currentColor" d="M8 5.14v13.72a1 1 0 001.55.83l11-6.86a1 1 0 000-1.66l-11-6.86a1 1 0 00-1.55.83z"/>
                        </svg>
                        Play Demo
                      </>
                    )}
